...
void OLED_ClearLine(uint8_t startLine, uint8_t endLine, uint8_t startColumn, uint8_t endColumn)
{
    for (uint8_t line = startLine; line <= endLine; line++)
    {
        OLED_Set_Pos(startColumn, line); // Set the position to the specified line and starting column
        for (uint8_t column = startColumn; column <= endColumn; column++)
        {
            OLED_WR_Byte(0x00, OLED_DATA); // Write empty data to clear the column
        }
    }
}

void OLED_ClearRegion_1(uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
    ESP_LOGI(TAG, "清除局部");
    uint8_t i, n;
    ...
    {
        OLED_WR_Byte(0xb0 + i, OLED_CMD);
        OLED_WR_Byte(((col_start & 0xf0) >> 4) | 0x10, OLED_CMD);
